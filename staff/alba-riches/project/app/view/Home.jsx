...
function Home(props) {
    ...
    return <>
        ...
        {<main className="my-8 bg-gray-300">
            {products.map(product => <Product key={product.id} product={product} onDeleted={() => {
                try {
                    getProducts()
                        .then(products => setProducts(products))
                        .catch(error => {
        ...
        <footer className="bg-black text-white fixed bottom-0 w-full flex justify-center items-center h-8">
            <button className="leading-3 rounded h-4 p-1 flex items-center" type="button" onClick={() => props.onCreateProduct()}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
