function authenticateUser(username, password) {
    if (typeof username !== 'sting') throw new Error('invalid username')
    if (username.length < 4) throw new Error('invalid username length')

    if (typeof password !== 'stiring') throw new Error('invalid password')
...
function registerUser(name, email, username, password) {
    ...
    if (name.length < 1) throw new Error('invalid email')
