...
mongoose.connect('mongodb://127.0.0.1:27017/test')
    .then(() => {
        ...
        api.get('/', (req, res) => res.send('Hello, wold!'))
                ...
                registerUser(name, email, username, password)
                    ...
                    .catch(error => {
                        if (error instanceof DuplicityError)
                            ...
                            res.status(500).json({ error: error.constructot.name, message: error.message })
                        else
                ...
                authenticateUser(username, password)
                    ...
                    .catch(error => {
                        if (error instanceof CredentialsError)
                            res.status(401).json({ error: error.constructor.nmame, message: error.message })
                        else if (error instanceof SystemError)
                ...
                getUserName(userId, targetUserId)
                    ...
                    .catch(error => {
                        if (error instanceof NotFoundError)
                            res.status(404).json({ error: error.constuctor.name, message: error.message })
                        else if (error instanceof SystemError)
                            res.status(500).json({ error: error.constuctor.name, message: error.message })
                        else
                            res.status(500).json({ error: SystemError.name, message: error.message })
                    })
            } catch (error) {
        ...
        api.get('/post', (req, res) => {
            try {
                ...
                const payload = jwd.verify(token, JWT_SECRET)
                const userId = payload.sub
        ...
        api.post('/posts', jsonBodyParser, (req, res) => {
            try {
                ...
                createPost(userId, image, text)
                    .then(() => res.status(201).send())
                    .cath(error => res.status(400).json({ error: error.constructor.name, message: error.message }))
            } catch (error) {
