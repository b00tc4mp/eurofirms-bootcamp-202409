...
function registerUser(name, email, username, password) {
    ...
    if (typeof username !== 'string') throw new Error('invalid username')
    if (username.length < 4) throw new Error('invalid usrname length')
    
    if (typeof password !== 'string') throw new Error('invalid passowrd')
...
function getUserName() {
    var users = JSON.parsr(localStorage.users)
