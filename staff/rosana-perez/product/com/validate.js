...
const validate = {
    name(name) {
        ...
        if (name.length < 4) throw new ValidationError('invalid name length')
    },

    email(email) {
        if (typeof email !== 'string') throw new ValidationError('invalid email')
        if (email.length < 6) throw new ValidationError('invalid mail length')
        if (!email.includes('@')) throw new ValidationError('invalidad email format')
    ...
    username(username) {
        ...
        if (username.length < 4) throw ValidationError('invalid username length')
    },
